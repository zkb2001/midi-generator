...
def apply_rope(x, positions):
    """应用RoPE（Rotary Position Embedding）到输入张量
    
    Args:
        x: 输入张量，形状为 (batch_size, num_heads, seq_len, head_dim) 或 (batch_size, seq_len, head_dim)
        positions: 位置索引，形状为 (seq_len,) 或标量
    ...
    if len(original_shape) == 4:
        ...
        batch_size, seq_len, head_dim = original_shape
        x_reshaped = x
        reshape_back = False
    
    # 确保head_dim是偶数（RoPE需要成对处理）
    assert head_dim % 2 == 0, f"head_dim必须是偶数，当前为{head_dim}"
    
    # 计算频率
    # theta_i = 10000^(-2i/d) for i in [0, 2, 4, ..., d-2]
    i = np.arange(0, head_dim, 2).astype(np.float32)
    theta = 1.0 / (10000 ** (i / head_dim))
    ...
    # 扩展positions以匹配batch维度
    if positions.ndim == 1 and len(positions) == seq_len:
        # positions是序列位置 [0, 1, 2, ..., seq_len-1]
        pos_angles = positions[:, None] * theta[None, :]  # (seq_len, head_dim//2)
    else:
        # positions是单个位置或需要广播
        pos_angles = np.array(positions)[:, None] * theta[None, :]
    
    # 计算cos和sin
    cos = np.cos(pos_angles)  # (seq_len, head_dim//2)
    sin = np.sin(pos_angles)  # (seq_len, head_dim//2)
    
    # 将x分成两部分：偶数索引和奇数索引
    x1 = x_reshaped[:, :, 0::2]  # (batch*heads, seq_len, head_dim//2) - 偶数索引
    x2 = x_reshaped[:, :, 1::2]  # (batch*heads, seq_len, head_dim//2) - 奇数索引
    
    # 扩展cos和sin以匹配batch维度
    # cos和sin的形状是(seq_len, head_dim//2)，需要扩展为(batch*heads, seq_len, head_dim//2)
    cos_expanded = cos[None, :, :]  # (1, seq_len, head_dim//2)
    sin_expanded = sin[None, :, :]  # (1, seq_len, head_dim//2)
    ...
    # 恢复原始形状
    if reshape_back:
        x_rot = x_rot.reshape(original_shape)
...
class MultiHeadAttention:
    ...
    def __init__(self, d_model, num_heads):
        ...
        # 确保head_dim是偶数（RoPE要求）
        assert self.head_dim % 2 == 0, f"head_dim必须是偶数，当前为{self.head_dim}"
    ...
    def forward(self, query, key, value, mask=None):
        ...
        # 保存应用RoPE前的Q和K（用于反向传播）
        Q_before_rope = Q.copy()
        K_before_rope = K.copy()
        
        # 应用RoPE到Q和K
        # 位置索引：对于query和key，使用它们各自的位置
        q_positions = np.arange(seq_len_q)
        k_positions = np.arange(seq_len_k)
        
        Q = apply_rope(Q, q_positions)
        K = apply_rope(K, k_positions)
        ...
        self.key = key
        self.value = value
        self.Q = Q  # 保存应用RoPE后的Q
        self.K = K  # 保存应用RoPE后的K
        self.V = V  # 保存多头格式的V
        self.Q_before_rope = Q_before_rope  # 保存应用RoPE前的Q（用于反向传播）
        self.K_before_rope = K_before_rope  # 保存应用RoPE前的K（用于反向传播）
...
class MusicTransformer:
    ...
    def __init__(self, vocab_size, d_model=256, num_heads=8, num_layers=6, d_ff=1024, max_len=5120):
        ...
        # 注意：不再使用绝对位置编码，位置信息由RoPE在注意力层中编码
        # self.pos_encoding = positional_encoding(max_len, d_model)  # 已移除
    ...
    def embed(self, token_ids):
        """嵌入层（不再添加位置编码，位置信息由RoPE在注意力层中处理）"""
        batch_size, seq_len = token_ids.shape
        embedded = self.embedding[token_ids]
        # 注意：不再添加位置编码，因为使用RoPE在注意力层中编码位置信息
        return embedded
...
def train_model(sequences, vocab, id_to_token, time_resolution, epochs=30, batch_size=4, 
                samples_per_epoch=200, d_model=256, num_heads=8, num_layers=4, d_ff=1024):
    """训练模型"""
    print(f"\n开始训练模型...")
...
def main_train_and_generate():
    ...
    model = train_model(sequences, vocab, id_to_token, time_resolution, 
                       epochs=20, batch_size=4, samples_per_epoch=200)
